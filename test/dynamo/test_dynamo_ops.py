...
class TestTensorMetaProp(torch._dynamo.test_case.TestCase):
    """Inplace ops propagate tensor metadata correctly during Dynamo tracing."""
    ...
    def test_inplace_ops_propagate_requires_grad_metadata(self, device, dtype, op):
        """An inplace op on a non-grad tensor using a grad-requiring arg must
        propagate requires_grad/dtype identically in eager and compiled modes.
        If metadata is traced wrong, code branching on requires_grad (e.g.
        custom autograd) silently takes the wrong path.
        """
        ...
        class CustomAutograd(torch.autograd.Function):
            ...
            def backward(ctx, grad_out):
                # Obviously-wrong gradient: detects composite-implicit-autograd
                # vs custom backward.
                (x,) = ctx.saved_tensors
                return torch.full_like(x, 123.0)

        # Iterate sample_inputs directly (listing it breaks TrackedInputIter).
        for sample in op.sample_inputs(device, dtype, requires_grad=False):
            if sample.broadcasts_input or sample.input.numel() == 0:
                continue

            # scatter with reduce modes has no backward.
            if op.name == "scatter" and "reduce" in sample.kwargs:
                continue

            torch._dynamo.reset()

            # x: requires_grad=False; one arg: requires_grad=True.
            ...
            # Check metadata is propagated at compile time.
            ...
            # requires_grad propagated at runtime.
            self.assertEqual(
            ...
            # Gradients match (metadata was propagated during tracing).
            self.assertEqual(
