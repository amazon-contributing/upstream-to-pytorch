...
"""
Op-behavior tests under Dynamo tracing / ``torch.compile``.

- ``TestTensorMetaProp``: inplace ops propagate tensor metadata
  (requires_grad, dtype) correctly during tracing.
- ``TestCompile*``: run each OpInfo op through ``torch.compile`` on the device
  and compare against a CPU reference (the same op run eagerly on CPU). One
  suite per upstream source file: OpInfo (test_ops), unary/binary ufuncs,
  reductions, foreach, linalg, nn.functional, and gradients.

All ``TestCompile*`` suites share ``_CompileConsistencyMixin`` and honor
``PYTORCH_TEST_MAX_OPS`` (op-count cap) and ``PYTORCH_TEST_COMPILE_BACKEND``
(default ``"aot_eager"``).
"""
...
class TestCompileReductionOpInfo(
    ...
    @skipOps(
        "TestCompileReductionOpInfo",
        "test_compile_reduction",
...
class TestCompileForeachOpInfo(
    ...
    @skipOps(
        "TestCompileForeachOpInfo",
        "test_compile_foreach",
...
class TestCompileNNFunctionalOpInfo(
    ...
    @skipOps(
        "TestCompileNNFunctionalOpInfo",
        "test_compile_nn_functional",
...
class TestCompileOpInfoGrad(_CompileConsistencyMixin, TestGradients):
    ...
    @skipOps(
        "TestCompileOpInfoGrad",
        "test_compile_fn_grad",
...
# Per-suite skip sets
test_compile_matches_eager_skips: set = {
    # Uninitialized memory -> nondeterministic values (all dtypes).
    skip("empty"),
    skip("empty_like"),
    skip("empty_permuted"),
    skip("empty_strided"),
    ...
    # bfloat16 accumulation-order overshoot.
    skip("addcdiv", dtypes=(torch.bfloat16,)),
    skip("nn.functional.interpolate", "bicubic", dtypes=(torch.bfloat16,)),
    skip("nn.functional.interpolate", "trilinear", dtypes=(torch.bfloat16,)),
    # Complex pinv: compiled decomposition diverges from eager.
    skip("linalg.pinv", dtypes=(torch.complex64,)),
    skip("pinverse", dtypes=(torch.complex64,)),
}
test_compile_unary_ufunc_skips: set = set()
test_compile_binary_ufunc_skips: set = set()
test_compile_reduction_skips: set = set()
# NB: foreach OpInfos aren't in op_db, so skipOps can't target them; see
# _FOREACH_DTYPE_SKIPS below.
test_compile_foreach_skips: set = set()
test_compile_linalg_skips: set = {
    skip("linalg.pinv", dtypes=(torch.complex64,)),
}
test_compile_nn_functional_skips: set = {
    skip("nn.functional.embedding_bag", dtypes=(torch.bfloat16,)),
    skip("nn.functional.interpolate", "bicubic", dtypes=(torch.bfloat16,)),
    skip("nn.functional.interpolate", "linear", dtypes=(torch.bfloat16,)),
    skip("nn.functional.interpolate", "trilinear", dtypes=(torch.bfloat16,)),
}
test_compile_grad_skips: set = {
    # bfloat16 grad accumulation-order overshoot (norm layers).
    skip("_batch_norm_with_update", dtypes=(torch.bfloat16,)),
    skip("_native_batch_norm_legit", dtypes=(torch.bfloat16,)),
    skip("native_batch_norm", dtypes=(torch.bfloat16,)),
    skip("nn.functional.batch_norm", dtypes=(torch.bfloat16,)),
    skip("nn.functional.instance_norm", dtypes=(torch.bfloat16,)),
    # Overlapping-view grad is ill-defined; compile counts the overlap
    # differently than eager (dtype-independent).
    skip("as_strided", "partial_views"),
    # Harness loss reduction (sum) is unimplemented for ComplexHalf on CPU.
    skip("chalf"),
}


# Foreach OpInfos live in the foreach-specific op dbs, not ``op_db``, so
# ``skipOps`` (which only searches ``op_db``) can't target them. Skip these
# op/dtype combos in-test instead. Each raises the *same* error in eager CPU
# (unsupported dtype/sample), so it isn't an aot_eager-vs-CPU divergence.
_FOREACH_DTYPE_SKIPS = {
    "_foreach_addcdiv": (torch.int64,),  # integer division no longer supported
    "_foreach_ceil": (torch.complex64,),  # not supported for complex
    "_foreach_clamp_max": (torch.complex64,),
    "_foreach_clamp_min": (torch.complex64,),
    "_foreach_erf": (torch.complex64,),
    "_foreach_erfc": (torch.complex64,),
    "_foreach_floor": (torch.complex64,),
    "_foreach_frac": (torch.complex64, torch.int64),
    "_foreach_lerp": (torch.int64,),
    "_foreach_lgamma": (torch.complex64,),
    "_foreach_max": (torch.complex64,),
    "_foreach_maximum": (torch.complex64,),
    "_foreach_minimum": (torch.complex64,),
    "_foreach_norm": (torch.int64,),  # needs a float/complex input
    "_foreach_round": (torch.complex64,),
    "_foreach_sign": (torch.complex64,),
    # A sample passes a bool scalar arg; ``sub`` with a bool is unsupported for
    # every tensor dtype, so skip the op outright.
    "_foreach_sub": (
        torch.bfloat16,
        torch.complex64,
        torch.float32,
    ...
    "_foreach_trunc": (torch.complex64,),
}
...
# Elementwise ufunc / reduction op lists (aten-only; see _aten_only).
_unary_ufunc_subset = _aten_only(unary_ufuncs)
_binary_ufunc_subset = _aten_only(binary_ufuncs)
_reduction_subset = _aten_only(reduction_ops)
...
_foreach_subset = _foreach_op_db

# test_linalg.py / test_nn.py aren't OpInfo-driven; use the named op_db slices.
_linalg_ops = [op for op in op_db if op.name.startswith("linalg")]
_nn_functional_ops = [op for op in op_db if op.name.startswith("nn.functional")]
_linalg_subset = _linalg_ops
_nn_functional_subset = _nn_functional_ops

# Grad suite: only ops declaring autograd support.
_grad_ops = [op for op in op_db if op.supports_autograd]
_grad_subset = _grad_ops
...
class _CompileConsistencyMixin:
    """Shared harness: compiled op vs a CPU eager reference.
...
class TestCompileOpInfo(_CompileConsistencyMixin, torch._dynamo.test_case.TestCase):
    ...
    Forward only; generic compile-path coverage over every aten op's
    ``sample_inputs``, across ``_FORWARD_DTYPES``.
    """
...
class TestCompileForeachOpInfo(
    ...
    def test_compile_foreach(self, device, dtype, op):
        if dtype in _FOREACH_DTYPE_SKIPS.get(op.name, ()):
            self.skipTest(
                f"{op.name}/{dtype}: unsupported sample/dtype (raises in eager too)"
            )
        samples = op.sample_inputs(device, dtype, requires_grad=False)
        self._compare_forward(op, device, dtype, samples, _tol_default)
