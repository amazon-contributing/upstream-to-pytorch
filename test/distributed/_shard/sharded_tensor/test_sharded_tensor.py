...
class TestShardParameter(ShardedTensorTestBase):
    ...
    def test_shard_parameter(self):
        ...
        fc = torch.nn.Linear(12, 12).to(torch.device(DEVICE_TYPE, self.rank))
    ...
    def test_shard_parameter_errors(self):
        ...
        with self.assertRaisesRegex(ValueError, "not a contiguous Tensor"):
            fc.bias = torch.rand(10, 10).to(torch.device(DEVICE_TYPE, self.rank)).t()
            shard_parameter(fc, "bias", spec)
...
class TestShardTensor(ShardedTensorTestBase):
    ...
    def test_shard_tensor(self):
        ...
        tensor = torch.rand(12, 12).to(torch.device(DEVICE_TYPE, self.rank))
    ...
    def test_shard_tensor_with_empty_shard(self):
        ...
        tensor = torch.rand(9, 12).to(torch.device(DEVICE_TYPE, self.rank))
        st = _shard_tensor(tensor, spec)
    ...
    def test_shard_tensor_errors(self):
        ...
        with self.assertRaisesRegex(ValueError, "not a contiguous Tensor"):
            tensor_t = torch.rand(12, 12).to(torch.device(DEVICE_TYPE, self.rank)).t()
...
class TestShardedTensorChunked(ShardedTensorTestBase):
    ...
    def test_gather_even(self) -> None:
        ...
        if self.rank == dst:
            full_tensor = torch.zeros(
                h,
                w,
                device=torch.device(DEVICE_TYPE, dst),
            )
    ...
    def test_create_sharded_tensor_with_rand(self):
        ...
        expected_h = 2
        expected_device = torch.device(DEVICE_TYPE, self.rank)
        dtype = torch.double
    ...
    def test_multiple_local_shards(self):
        ...
        for local_shard in local_shards:
            self.assertEqual(
                torch.device(DEVICE_TYPE, self.rank), local_shard.tensor.device
            )
...
class TestShardedTensorEnumerable(ShardedTensorTestBase):
    ...
    def test_grid_sharding(self):
        ...
        self.assertEqual(torch.device(DEVICE_TYPE, self.rank), local_shard.tensor.device)
    ...
    def test_gather_even(self) -> None:
        ...
        dst = 0
        if self.rank == dst:
            full_tensor = torch.zeros(h, w, device=torch.device(DEVICE_TYPE, dst))
        st.gather(dst, full_tensor)
    ...
    def test_sharded_tensor_to_cuda(self):
        ...
        h, w = 10, 20
        # Accelerator sharded tensor should return a new ShardedTensor, but same
        # local shards(no movements)
        st_dev = sharded_tensor.zeros(spec, h, w)
        new_st_dev = st_dev.cuda()
        self.assertTrue(st_dev is not new_st_dev)
        self.assertTrue(st_dev.local_tensor() is new_st_dev.local_tensor())
    ...
    def test_sharded_tensor_to_test(self):
        ...
        st_dev = st_cpu.to(device=torch.device(DEVICE_TYPE))
        self.assertEqual(st_dev.local_tensor().device.type, DEVICE_TYPE)
        # non-kwarg device to
        st_dev = st_cpu.to(torch.device(DEVICE_TYPE))
        self.assertEqual(st_dev.local_tensor().device.type, DEVICE_TYPE)
        st_cpu = st_dev.to(torch.device("cpu"))
        self.assertEqual(st_cpu.local_tensor().device.type, "cpu")
        # with string like device conversion
        st_cpu = st_dev.to("cpu")
        self.assertEqual(st_cpu.local_tensor().device.type, "cpu")
        st_dev = st_cpu.to(DEVICE_TYPE)
        self.assertEqual(st_dev.local_tensor().device.type, DEVICE_TYPE)
        # with int like device conversion
        st_cpu = st_dev.to("cpu")
        self.assertEqual(st_cpu.local_tensor().device.type, "cpu")
        st_dev = st_cpu.to(self.rank)
        self.assertEqual(st_dev.local_tensor().device.type, DEVICE_TYPE)

        # test tensor to
        dev_tensor = torch.randn(3, 4, dtype=torch.float16, device=DEVICE_TYPE)
        st_dev = st.to(dev_tensor)
        self.assertFalse(st_dev is st)
        self.assertEqual(st_dev.dtype, torch.float16)

        dev_tensor = torch.randn(3, 4, dtype=torch.float16, device=f"{DEVICE_TYPE}:2")
        st_dev = st.to(dev_tensor)
        self.assertEqual(st_dev.dtype, torch.float16)
        ...
        st_dev_32 = st_cpu_16.to(DEVICE_TYPE, torch.float32)
        self.assertEqual(st_dev_32.dtype, torch.float32)
        self.assertEqual(st_dev_32.local_tensor().device.type, DEVICE_TYPE)
    ...
    def test_sharded_tensor_device(self):
        ...
        # local shards(no movements)
        st = sharded_tensor.zeros(spec, h, w)
        current_device = torch.device(torch.accelerator.current_accelerator().type, torch.accelerator.current_device_idx())
...
class TestShardedTensorFromLocalTensor(ShardedTensorTestBase):
    def _generate_st_from_chunk_local_tensor(self, st_size, sharding_spec):
        ...
        for shard_metadata in tensor_meta.shards_metadata:
            ...
            if rank == self.rank:
                local_tensor = torch.rand(shard_metadata.shard_sizes).to(torch.device(DEVICE_TYPE, device))
    ...
    def test_init_from_local_tensor_errors(self):
        ...
        st_size = [24, 12]
        local_tensor = torch.rand(*st_size).to(torch.device(DEVICE_TYPE, self.rank))
...
class TestShardedTensorCustomOps(ShardedTensorTestBase):
    ...
    def test_custom_op_override(self):
        t = torch.rand(10, 10).to(torch.device(DEVICE_TYPE, self.rank))
        ...
        m = torch.nn.Linear(32, 16).to(torch.device(DEVICE_TYPE, self.rank))
        shard_parameter(m, "weight", spec)

        result = m(torch.rand(15, 32).to(torch.device(DEVICE_TYPE, self.rank)))
